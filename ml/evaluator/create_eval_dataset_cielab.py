#!/usr/bin/env python3
"""
Create JSONL evaluation dataset from test samples using CIELAB format.

Usage:
    python -m ml.evaluator.create_eval_dataset_cielab
    python -m ml.evaluator.create_eval_dataset_cielab --limit 10
"""
...
from ml.evaluator.config import (
    ...
    SYSTEM_PROMPT_CIELAB, USER_PROMPT_TEMPLATE_CIELAB
)
...
def rgb_to_lab(rgb):
    ...
    lab = sk_color.rgb2lab(rgb_normalized)[0][0]
...
def hex_to_lab(hex_code):
    """
    Convert hex color code to CIELAB format string.
...
def create_dataset(output_dir: str = None, limit: int = None):
    ...
    print(f"=== Creating Evaluation Dataset (CIELAB) ===")
    ...
    for i, (query_words, gt_rgb_palette) in enumerate(tqdm(zip(test_names, test_palettes), total=len(test_names), desc="Processing")):
        ...
        # Format examples - colors are already in CIELAB format in the database
        output_lines = []
        ...
        user_content = USER_PROMPT_TEMPLATE_CIELAB.format(query=query_str, examples=examples)
        ...
        # Convert ground truth palette to CIELAB format
        gt_lab_palette = [hex_to_lab(hex_code) for hex_code in gt_hex_palette.to_hex_list()]
    ...
    jsonl_filename = f"eval_prompts_cielab_{timestamp}.jsonl"
...
def main():
    parser = argparse.ArgumentParser(
        description="Create JSONL evaluation dataset from test samples using CIELAB format.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    python -m ml.evaluator.create_eval_dataset_cielab
    python -m ml.evaluator.create_eval_dataset_cielab --limit 10
    python -m ml.evaluator.create_eval_dataset_cielab --output-dir custom/path
        """
    )
    parser.add_argument(
        "--output-dir",
